require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe EdgesController do

  # This should return the minimal set of attributes required to create a valid
  # Edge. As you add validations to Edge, be sure to
  # update the return value of this method accordingly.
  def valid_attributes
    {from_id: 1, to_id: 1}
  end

  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # EdgesController. Be sure to keep this updated too.
  def valid_session
    {}
  end

  describe "GET index" do
    it "assigns all edges as @edges" do
      edge = Edge.create! valid_attributes
      get :index, {}, valid_session
      assigns(:edges).should eq([edge])
    end
  end

  describe "GET show" do
    it "assigns the requested edge as @edge" do
      edge = Edge.create! valid_attributes
      get :show, {:id => edge.to_param}, valid_session
      assigns(:edge).should eq(edge)
    end
  end

  describe "GET new" do
    it "assigns a new edge as @edge" do
      get :new, {}, valid_session
      assigns(:edge).should be_a_new(Edge)
    end
  end

  describe "GET edit" do
    it "assigns the requested edge as @edge" do
      edge = Edge.create! valid_attributes
      get :edit, {:id => edge.to_param}, valid_session
      assigns(:edge).should eq(edge)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Edge" do
        expect {
          post :create, {:edge => valid_attributes}, valid_session
        }.to change(Edge, :count).by(1)
      end

      it "assigns a newly created edge as @edge" do
        post :create, {:edge => valid_attributes}, valid_session
        assigns(:edge).should be_a(Edge)
        assigns(:edge).should be_persisted
      end

      it "redirects to the created edge" do
        post :create, {:edge => valid_attributes}, valid_session
        response.should redirect_to(Edge.last)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved edge as @edge" do
        # Trigger the behavior that occurs when invalid params are submitted
        Edge.any_instance.stub(:save).and_return(false)
        post :create, {:edge => {}}, valid_session
        assigns(:edge).should be_a_new(Edge)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        Edge.any_instance.stub(:save).and_return(false)
        post :create, {:edge => {}}, valid_session
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested edge" do
        edge = Edge.create! valid_attributes
        # Assuming there are no other edges in the database, this
        # specifies that the Edge created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        Edge.any_instance.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, {:id => edge.to_param, :edge => {'these' => 'params'}}, valid_session
      end

      it "assigns the requested edge as @edge" do
        edge = Edge.create! valid_attributes
        put :update, {:id => edge.to_param, :edge => valid_attributes}, valid_session
        assigns(:edge).should eq(edge)
      end

      it "redirects to the edge" do
        edge = Edge.create! valid_attributes
        put :update, {:id => edge.to_param, :edge => valid_attributes}, valid_session
        response.should redirect_to(edge)
      end
    end

    describe "with invalid params" do
      it "assigns the edge as @edge" do
        edge = Edge.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Edge.any_instance.stub(:save).and_return(false)
        put :update, {:id => edge.to_param, :edge => {}}, valid_session
        assigns(:edge).should eq(edge)
      end

      it "re-renders the 'edit' template" do
        edge = Edge.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Edge.any_instance.stub(:save).and_return(false)
        put :update, {:id => edge.to_param, :edge => {}}, valid_session
        response.should render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested edge" do
      edge = Edge.create! valid_attributes
      expect {
        delete :destroy, {:id => edge.to_param}, valid_session
      }.to change(Edge, :count).by(-1)
    end

    it "redirects to the edges list" do
      edge = Edge.create! valid_attributes
      delete :destroy, {:id => edge.to_param}, valid_session
      response.should redirect_to(edges_url)
    end
  end

end
